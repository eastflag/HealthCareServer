...
<script type="text/javascript">
	...
	$('#height_page').on('pageshow',function(){
		...
		window.pSwipe = Swipe(document.getElementById('pSwipe'), {
			...
			callback: function(idx, element) {
				chart_index = idx; //swipe left 로 back 했을때 상세 이력 바뀌지 않음 수정 // 2015.06.11 아우라 오류 수정요청으로 인해 수정
			//console.log(" ::: swipe callback index :: :: "+idx );
			},
			transitionEnd: function(idx, element) {
				//console.log(" ::: swipe transitionEnd  index :: :: "+idx );
	...
	function initPage(idx){
		...
		if(typeObj.detail){
			$(".ui-btn-right").show();
			//console.log("init page show ::: "+idx);
		}else{
			$(".ui-btn-right").hide();
			//console.log("init page hide ::: "+idx);
		}
	...
	function setDetailPage(idx){
		...
		//console.log("DetailPage index ::: "+idx);
		switch(idx){
	...
	function setDetailYearPage(selYear){
		...
		if (selYear != 'yearSelect') {
			switch(chart_index){
				case 0: 
					setUserYearData("GetHeightHistoryYear", selYear, subDetailYear); 
					break;// 신장이력	
				case 1:
					setUserYearData("GetWeightHistoryYear", selYear, subDetailYear); 
					break;// 체중이력
				case 2: 
					setUserYearData("GetBmiHistoryYear",    selYear, subDetailYear);
					break; //체형이력 
			}
		}
	}
...
// END data 바인딩 ***************************************************************************
	...
	function setDetail(){
		...
		//console.log(" ::기종   ::: " + deviceType);
		
		
		//console.log(" ::111 이력 ::: " +chart_index);
	...
	function subDetail(jo) {
	   ...
	   // console.log( " SubDetail Val 값 "+JSON.stringify( jo));
	    $("#tooltip1b2").hide();
	    var val = jo.value;
...
// 	    }
        ...
        $("#dialog").load(subLoadPage, function () {
            	...
            	if (vM[j] != "A") {
                    for (var j = 0; j < 4; j++) {          	
	                    ...
	                    if ((val.list[i].date).substr(0, 7) != vM[j]) {
	                        ...
	                        vSchool[j] = "0";
	                        vLocal[j] = "0";
	                        //console.log(j+"번째  substr(0, 7)   vMe "+vMe[j]); 
	                        //console.log(j+"번째  substr(0, 7)   vClass "+vClass[j]); 
	                        //console.log(j+"번째  substr(0, 7)   vschool "+vSchool[j]); 
	                        //console.log(j+"번째  substr(0, 7)   vLocal "+vLocal[j]); 
	                       // 
	                    } else {
	                        ...
	                        //console.log(j+"번째    vMe "+vMe[j]); 
	                        //console.log(j+"번째    vClass "+vClass[j]); 
	                        //console.log(j+"번째    vschool "+vSchool[j]); 
	                        //console.log(j+"번째    vLocal "+vLocal[j]); 
	                        break;
	                    }
	                }
            	}
           ...
           /* 
            for(var i = 0; i < len; i++){
            	 vMe[i] = val.list[i].value == "undefined" ? "0" : val.list[i].value;
            	 vClass[i] = val.list[i].averageOfClass == "undefined" ? "0" : val.list[i].averageOfClass;
            	  vSchool[i] = val.list[i].averageOfSchool == "undefined" ? "0" : val.list[i].averageOfSchool;
            	   vLocal[i] = val.list[i].averageOfLocal == "undefined" ? "0" : val.list[i].averageOfLocal;
                     //console.log(j+"번째    vMe "+vMe[i]); 
                     //console.log(j+"번째    vClass "+vClass[i]); 
                     //console.log(j+"번째    vschool "+vSchool[i]); 
                     //console.log(j+"번째    vLocal "+vLocal[i]); 
...
//             }
            ...
            // console.log ("vSchool::::::: " +    vSchool);
            // console.log ("vSchoolYear :::: " + vSchoolYear);
            // console.log ("schoolFirstIdx ::: " +schoolFirstIdx);
            // console.log ("schoolLastIdx ::: "+ schoolLastIdx);
	...
	function subDetailYear(jo) {
        ...
        if (chart_index == 0 || chart_index == 1) {
            ...
            $("#dialog").load(subLoadPage, function () {
                ...
                // console.log ("올해 vSchoolYear :::: " + vSchoolYear);
                // console.log ("올해 schoolFirstIdx ::: " +schoolFirstIdx);
                // console.log ("올해 schoolLastIdx ::: "+ schoolLastIdx);
