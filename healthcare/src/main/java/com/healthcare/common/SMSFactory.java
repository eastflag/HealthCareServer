...
public final class SMSFactory {
	...
	private String testFlag = null; // 테스트페이지 요청임. sms는 전송하지 않으나 단순한 테스트 페이지 용도. null(운영) or "Y"(테스트)
	...
	public SMSFactory (String tempKey, String phone) throws Exception {
		...
		user_id = base64Encode("aurasystem");	//아이디
		secure = base64Encode("32c8990212758a346a50acbe6a106ee0");// 인증키
		msg = base64Encode(nullcheck(setMessage(tempKey), ""));
		receive_phone_number = base64Encode(nullcheck(phone, ""));

		first_phone_number = base64Encode(nullcheck("1544", ""));    
		second_phone_number = base64Encode(nullcheck("1284", "")); 
		third_phone_number = base64Encode(nullcheck("", "")); 
	...
	private String setMessage(String msg){
		return "[스마트 건강지킴이] 요청한 인증키는 [ "+msg+" ] 입니다.";
	}
