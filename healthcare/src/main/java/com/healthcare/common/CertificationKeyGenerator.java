...
public class CertificationKeyGenerator {
	 ...
	 * @param phone
	 */
	public void tempKeyGenerator (SMSCertService smsCertService, String phone) throws Exception {
		
		//인증키 생성
		String tempKey = tempKeyGenarator(phone);
		logger.info("SMS 인증 : 요청 mdn [" + phone + "] 발행키 [" + tempKey + "]");

		//이전 인증키 삭제
		smsCertService.deleteSMSCertKeyInfo(phone);
		
		//인증키및 전화번호 파라미터화
		SMSCert smsCert = new SMSCert();
		smsCert.setMdn(phone);
		smsCert.setCertificationNumber(tempKey);
		...
		if (smsFactory.Send()) {
			...
			logger.info("SMS 전송 완료");
			smsCertService.insertSMSCertKeyInfo(smsCert);		
		}
	}
	 ...
	 * @return
	 */
	public boolean isCorrectCertifiKey(SMSCertService smsCertService, String phone , String inputKey) {
		
		//db에서 dbKey를 가져와 저장할 임시변수
		String dbKey = "";
		
		//암호화된 전화번호로 임시키 가져옴
		dbKey = smsCertService.getSMSCertKeyInfo(phone).getCertificationNumber();
